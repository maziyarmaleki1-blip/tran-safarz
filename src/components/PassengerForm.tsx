...
const PassengerForm: React.FC<PassengerFormProps> = ({
  ...
  // Field order based on RTL/LTR
  const passengerFields = isRTL
    ? ['firstName', 'lastName', 'nationalId', 'birthDate']
    : ['birthDate', 'nationalId', 'lastName', 'firstName'];

  const accountFields = isRTL
    ? ['mobile', 'password', 'confirmPassword', 'submit']
    : ['submit', 'confirmPassword', 'password', 'mobile'];
  ...
  const renderAccountField = (field: string) => {
    const textAlign = isRTL ? 'text-right' : 'text-left';
    ...
    if (field === 'submit') {
      return (
        <Button
          ...
          className="h-10 bg-emerald-500 hover:bg-emerald-600 text-white font-medium rounded-lg self-end"
        >
